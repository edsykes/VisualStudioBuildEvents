...
namespace Company.HookIntoBuildEvents
{
    ...
    public sealed class HookIntoBuildEventsPackage : Package
    {
        ...
        /// </summary>
        public HookIntoBuildEventsPackage()
        {
        ...
        protected override void Initialize()
        {
            ...
            string context = "SYKES";

            var dte = (DTE)GetService(typeof(DTE));
            dte.Events.BuildEvents.OnBuildBegin += (scope, action) =>
            {
                Debug.WriteLine("Build Begins",  context);
                Debug.WriteLine(scope, context);
                Debug.WriteLine(action, context);
            };

            dte.Events.BuildEvents.OnBuildDone += (scope, action) =>
            {
                Debug.WriteLine("Build End", context);
                Debug.WriteLine(scope, context);
                Debug.WriteLine(action, context);
            };

            dte.Events.BuildEvents.OnBuildProjConfigBegin += (project, config, platform, solutionConfig) =>
            {
